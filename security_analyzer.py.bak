...
class PureAISecurityAnalyzer:
    def __init__(self):
        ...
        # Supported file extensions (now includes PHP)
        self.extensions = [
            ...
            '.c', '.cpp', '.hpp', '.h', '.gradle', '.jsonnet', '.json5'
        ]
...
{content}"""
        ...
        try:
            ...
            fixed_content = re.sub(r'```(?:terraform|hcl|yaml|yml|php|json|xml|html)?\s*', '', fixed_content)
            fixed_content = re.sub(r'```\s*$', '', fixed_content)
            fixed_content = fixed_content.strip()

            # Basic syntax validation only
            is_valid = self.basic_validation(fixed_content, filename)
    ...
    def basic_validation(self, stripped, filename):
        """Very lightweight sanity checks by file type"""
        lower = filename.lower()
        if lower.endswith('.tf'):
            return 'resource' in stripped or 'provider' in stripped
        if lower.endswith(('.yaml', '.yml')):
            return 'apiVersion:' in stripped or 'kind:' in stripped
    ...
    def run(self):
        ...
        # Debug: list PHP files found
        for f in files:
            if f.endswith('.php'):
                print(f"👉 PHP file detected: {f}")

        all_issues = []

        for file_path in files:
            if os.path.getsize(file_path) < 10240:  # 10 KB limit
